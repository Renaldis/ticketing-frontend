      ...
      <table className="w-full text-left text-xs">
        <thead>
          <tr className="border-b border-[#464554]/40 text-[#908fa0] uppercase font-bold tracking-wider">
            ...
            <th className="pb-3.5">Event Title</th>
            <th className="pb-3.5">Ticket Tier</th>
            <th className="pb-3.5">Gross Amount</th>
          ...
          {orders.length === 0 ? (
            ...
            orders.map((ord) => (
              <tr key={ord.id} className="text-[#c7c4d7] hover:bg-[#1b1b23]/50 transition">
                <td className="py-4 font-mono text-[11px] text-[#4cd7f6]">{ord.id.substring(0, 8)}...</td>
                <td className="py-4 font-bold text-white">
                  {ord.user?.name || 'Customer'}
                  <span className="block text-[10px] font-normal text-[#908fa0]">{ord.user?.email}</span>
                </td>
                <td className="py-4 font-medium text-[#e4e1ed]">{ord.event?.title}</td>
                <td className="py-4 text-[#908fa0]">
                  {ord.orderItems?.map((i) => `${i.ticketCategory?.name} (x${i.quantity})`).join(', ')}
                </td>
                ...
                <td className="py-4 text-[#908fa0]">
                  {new Date(ord.createdAt).toLocaleString('id-ID', { dateStyle: 'short', timeStyle: 'short' })}
                </td>
              </tr>
            ))
          )}

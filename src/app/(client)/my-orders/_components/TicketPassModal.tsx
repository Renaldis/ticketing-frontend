...
import { CheckCircle2, AlertTriangle, X, Download } from 'lucide-react';
...
export const TicketPassModal = ({ ticket, user, onClose }: TicketPassModalProps) => {
  ...
  const handleDownloadQR = () => {
    ...
    const filename = `QR-Tiket-${(ticket.event?.title || 'Pass').replace(/[^a-zA-Z0-9]/g, '_')}-${ticket.orderId || 'Pass'}.png`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="premium-card rounded-2xl max-w-sm w-full p-8 text-center space-y-6 shadow-2xl relative border border-[#464554]/40">
        ...
        {/* PRINTABLE / CANVAS TARGET AREA */}
        <div className="space-y-6 p-4 rounded-xl bg-[#13131b]">
          ...
          <div className="bg-[#181824] p-4 rounded-xl border border-[#464554]/40 text-left text-xs space-y-2 text-[#c7c4d7]">
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="w-full btn-primary py-2.5 rounded-xl text-xs font-bold text-[#003640] flex items-center justify-center gap-2"
          >
            <Download className="w-4 h-4" />
            <span>Download QR Barcode (PNG)</span>
          </button>
          <button
            onClick={onClose}

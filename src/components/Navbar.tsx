          ...
          <div className="pt-3 border-t border-[#292932] flex items-center justify-between">
            {user ? (
              <>
                ...
                <button onClick={logout} className="text-xs text-rose-400 font-bold">Logout</button>
              </>
            ) : (
              <div className="grid grid-cols-2 gap-2 w-full">
                <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="text-center btn-secondary py-2 rounded-lg text-xs font-bold text-white">Login</Link>
                <Link href="/register" onClick={() => setMobileMenuOpen(false)} className="text-center btn-primary py-2 rounded-lg text-xs font-bold text-[#003640]">Sign Up</Link>
              </div>
            )}
          </div>
        </div>
